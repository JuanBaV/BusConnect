        ...
        private void btnGuardarRuta_Click(object sender, EventArgs e)
        {
            try
            {
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    ...
                    objramalesubi.Latitud = Convert.ToDouble(row.Cells["Lat"].Value);
                    objramalesubi.Longitud = Convert.ToDouble(row.Cells["Long"].Value);
                    int i = objnuser.N_Ubicaciones_register(objramalesubi);
                }
                //if (i == 1)
                //{
